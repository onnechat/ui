...
function ChartBrush({
  ...
  const [internalRange, setInternalRange] = React.useState<ChartBrushRange>(
    () => ({
  ...
  const lastCommittedRef = React.useRef<ChartBrushRange>(internalRange)
...
function useChartBrush<TData extends Record<string, unknown>>({
  data,
  defaultStartIndex = 0,
  defaultEndIndex,
}: {
  ...
  const [range, setRange] = React.useState<ChartBrushRange>({
  ...
  return {
    ...
    brushProps: {
      ...
      onChange: setRange,
    } satisfies Pick<ChartBrushProps, 'startIndex' | 'endIndex' | 'onChange'>,
  }
}
